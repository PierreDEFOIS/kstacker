...
def log_likelihood(
    ...
    if fast:
        if (state.cached_terms[walker_id][0] is None):
            log_likelihood_res, log_p, all_mask, all_pixel_indices = compute_log_likelihood(x, state, ts, size, scale, fwhm, treated_image)
            ...
            log_likelihood_res, log_p, all_mask, all_pixel_indices = recompute_log_likelihood(x, state, ts, size, scale, fwhm, treated_image)
            state.cached_terms[walker_id][1] = [log_likelihood_res, all_mask, all_pixel_indices]
    else:
        _, log_p, _, _ = compute_log_likelihood(x, state, ts, size, scale, fwhm, treated_image)
    return log_p